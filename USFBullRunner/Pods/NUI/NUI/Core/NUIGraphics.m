...
+ (UIImage*)backButtonWithClass:(NSString*)className
{
    ...
    if( [image respondsToSelector:@selector(resizableImageWithCapInsets:resizingMode:)] ){
        ...
        image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0, arrowWidth + 5, 0, cornerRadius + 5)];
    }
//    image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0, arrowWidth + 5, 0, cornerRadius + 5) resizingMode:UIImageResizingModeStretch];
...
+ (UIImage*)roundedRectImageWithClass:(NSString*)className layer:(CALayer*)layer
{
    ...
    if( [image respondsToSelector:@selector(resizableImageWithCapInsets:resizingMode:)] ){
        ...
        image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0, insetLength, 0, insetLength)];
    }
